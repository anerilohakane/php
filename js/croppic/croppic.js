...
(function (window, document) {
	...
	Croppic.prototype = {
		...
		bindImgUploadControl: function(){
            ...
            var formHtml = '<form class="' + that.id + '_imgUploadForm" style="visibility: hidden;">  <input type="file" class="ignore" name="img" id="' + that.id + '_imgUploadField">  </form>';
			...
			if( !$.isEmptyObject(that.croppedImg)){
				...
				that.cropControlRemoveCroppedImage.on('click',function(){ 
					...
					$(this).parents().find('.cropOutput').remove();
				});	
		...
		createModal: function(){
			...
			that.objW=480;
			that.objH =520;
			var modalHTML =  '<div id="croppicModal">'+'<div id="croppicModalObj" style="width:'+ that.objW +'px; height:'+ that.objH +'px; margin:0 auto; margin-top:'+ marginTop +'%; position: relative;"> </div>'+'</div>';
		...
		afterCrop: function (data) {
            ...
            if (response.status == 'success') {
                ...
                that.obj.append('<img style="width:100%; height:100%;" class="croppedImg" src="' + response.url + '">');
                that.obj.append('<input type="hidden" class="cropOutput" name="mult_img[]"  value="' + response.name + '">');
                $('#fundraiser_photo').attr('src',response.url);
