<?php $transcation_data1 = $this->session->userdata('transcation_data1');
if(!isset($transcation_data1) && empty($transcation_data1))
{
  redirect('home');
}?>
<!DOCTYPE html>
<html lang="en">
<head>
<title>Shivbandhan | Confirm</title>
  ...
  <section id="page-banner" class="page-banner" style="background-image: url('assets/site1/images/banner.jpg');"> 
    ...
    <div class="container">
      <div class="banner-dtl text-center">
        <h2 class="banner-heading">Confirm</h2>
      ...
      <div class="row">
        <div class="col-md-8">
          <div class="form-group">
            <p>Thank You.&nbsp;&nbsp;&nbsp;<?php echo isset($transcation_data1->f_name) && !empty($transcation_data1->f_name)?$transcation_data1->f_name.' '.$transcation_data1->m_name.' '.$transcation_data1->l_name:'' ; ?>&nbsp;&nbsp;&nbsp; Your payment has been Successfully.  </p>
            <p>Package Name:-  <b><?php echo isset($transcation_data1->membership_name) && !empty($transcation_data1->membership_name)?$transcation_data1->membership_name:'' ; ?></b></p>
             <p>Package Amount:-  <b><?php echo isset($transcation_data1->membership_amt) && !empty($transcation_data1->membership_amt)?$transcation_data1->membership_amt:'' ; ?></b></p>
            <p>Transction ID:-  <b><?php echo isset($transcation_data1->transcation_id) && !empty($transcation_data1->transcation_id)?$transcation_data1->transcation_id:'' ; ?></b></p>
            <p>Total Amount:- <b><?php echo isset($transcation_data1->membership_amt) && !empty($transcation_data1->membership_amt)?$transcation_data1->membership_amt:'' ; ?></b></p>
            <p>Deposite Date:- <b><?php echo isset($transcation_data1->modified_on) && !empty($transcation_data1->modified_on)?date('d-m-Y',strtotime($transcation_data1->modified_on)):'' ; ?></b></p>
          </div> 
          <div class="row">
            <div class="col-md-offset-3 col-md-9">
              <a href="<?php echo base_url();?>home" style="color: orangered;"><i class="fa fa-reply-all"></i> Go To Back</a>
            </div>
          </div>                  
        </div>
      </div>
